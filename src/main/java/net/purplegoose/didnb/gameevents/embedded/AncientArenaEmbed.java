...
@Slf4j
@AllArgsConstructor
public class AncientArenaEmbed implements IGameEvent {

    private final GameDataCache gameDataCache;

    public void sendAncientArenaEmbedIfHappening(ClientGuild guild, NotificationChannel channel, JDA client) {
        ...
        if (isEventAboutToStart(eventGameDataSet, time, weekday)) {
            ...
            textChannel.sendMessageEmbeds(createAncientArenaEmbed(timeZone, embedLeadTime)).queue();
        }
    }

    private MessageEmbed createAncientArenaEmbed(String timeZone, long leadTime) {
        ...
        embedBuilder.addField("World Bilefen", "Ancient Arena", false);
